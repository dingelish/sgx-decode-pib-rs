...
impl fmt::Display for platform_info_blob {
    fn fmt(&self, f: &mut fmt::Formatter) -> fmt::Result {
        ...
        writeln!(
            f,
            "\tsgx_epid_group_fags = {:02X}",
            self.sgx_epid_group_flags
        )?;
        test_and_print!(f, self.sgx_epid_group_flags, QE_EPID_GROUP_REVOKED);
        test_and_print!(
            f,
            self.sgx_epid_group_flags,
            PERF_REKEY_FOR_QE_EPID_GROUP_AVAILABLE
        );
        ...
        test_and_print!(f, tcb_eflags, PLATFORM_CONFIGURATION_NEEDED);
        ...
        test_and_print!(
            f,
            pse_eflags,
            PRIVRL_VER_FROM_PS_HW_PRV_KEY_RLVER_OUT_OF_DATE
        );
        writeln!(
            f,
            "\n\tlatest_equivalent_tcb_psvn: {:02X}",
            self.latest_equivalent_tcb_psvn.iter().format("")
        )?;
        writeln!(
            f,
            "\n\tlatest_pse_isvsvn: {:02X}",
            self.latest_pse_isvsvn.iter().format("")
        )?;
        writeln!(
            f,
            "\n\tlatest_psda_svn: {:02X}",
            self.latest_psda_svn.iter().format("")
        )?;
...
fn main() {
    ...
    assert_eq!(
        mem::size_of::<platform_info_blob>(),
        mem::size_of::<platform_info>()
    );
    assert_eq!(PSVN_SIZE, mem::size_of::<psvn_t>());

    let args: Vec<String> = env::args().collect();

    if args.len() != 2 || !input_is_ok(&args[1]) {
        display_help();
        return;
    }
